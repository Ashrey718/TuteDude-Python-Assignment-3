# TuteDude-Python-Assignment-3
## Description
This repository contains Python programs submitted as part of the TuteDude Python assignment 3.

Programs Included:-

### Task 1: Calculate Factorial Using a Function (Factorial.py)
- Defines a function named factorial that takes a number as an argument and calculates its factorial using a loop or recursion.
- Returns the calculated factorial.
- Calls the function with a sample number and prints the output.

### Task 2: Using the Math Module for Calculations. (Math_Module.py)
- Asks the user for a number as input.
- Uses the math module to calculate the:
  - Square root of the number
  - Natural logarithm (log base e) of the number
  - Sine of the number (in radians)
- Displays the calculated results.
